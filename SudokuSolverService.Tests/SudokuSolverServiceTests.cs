...
namespace SudokuSolver.Service.Tests
{
    [TestClass]
    public class SudokuSolverServiceTests
    {
        ...
        public void SolveBoardWithValidInput()
        {
            ...
            var response = new SudokuSolverService(new SudokuSolverCore()).Solve(new SudokuSolverRequest { Board = board });
            ...
            if (response.Status != SudokuSolverResponseStatus.Success) Assert.Fail("SolveBoardWithValidInput: Board could not be solved with valid input.");

            for (int row = 0; row < 9; row++)
            {
                for (int col = 0; col < 9; col++)
                {
                    if (response.Board[row, col] != expected[row, col])
